...
// Initialize UART0
// Baud rate is 115200 bits/sec
void UART_InitA(uint32_t Frequency, UART_enBaudRate_t baudrate ){
  float br = 0.0;                       // it will contains the result of the baud rate calculus
  uint32_t BusClockFrequency = 50000000;
  SYSCTL_RCGCUART_R |= 0x01;            // activate UART0
  SYSCTL_RCGCGPIO_R |= 0x01;            // activate port A
  RxFifo_Init();                        // initialize empty FIFOs
  TxFifo_Init();
  ...
  BusClockFrequency = UART_GetFreqFromPLL((UART_enBusFrequency_t)Frequency);
  br = (float) ((float)BusClockFrequency / (float)(16 * baudrate));   // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.1267)
  UART0_IBRD_R = (int)(br);  
  UART0_FBRD_R = (int) (((float)((float)br-(int)br))*64 + 0.5); // FBRD = int(0.1267 * 64 + 0.5) = 8
                                                // 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART0_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART0_IFLS_R &= ~0x3F;                // clear TX and RX interrupt FIFO level fields
                                        // configure interrupt for TX FIFO <= 1/8 full
...
uint32_t UART_GetFreqFromPLL(UART_enBusFrequency_t freq){
  ...
  {   
    ...
    case UART_enBus10MHz:
    {
      result = 10000000;
      break;
    }
    case UART_enBus16MHz:
    {
      result = 16000000;
      break;
    }
    case UART_enBus20MHz:
    {
      result = 20000000;
      break;
    }
    case UART_enBus25MHz:
    {
      result = 25000000;
      break;
    }
    case UART_enBus40MHz:
    {
      result = 40000000;
      break;
    }
    case UART_enBus50MHz:
    {
      result = 50000000;
      break;
    }
    case UART_enBus80MHz:
    {
      result = 80000000;
      break;
    }
